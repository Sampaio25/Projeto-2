function initSlide() {
    ...
    function getSlides() { return slides = document.querySelectorAll('.slide'); }

    function startSlide() {
        slideId = setInterval(() => { moveToNextSlide(); }, time)
    }

    function moveToNextSlide() {
        if (currentSlideIndex >= slides.length - 1) return;
        currentSlideIndex++;
        slider.style.transform = `translateX(${-slides[currentSlideIndex].clientWidth * currentSlideIndex}px)`
    ...
    sliderContainer.addEventListener('mouseenter', () => { clearInterval(slideId); })
